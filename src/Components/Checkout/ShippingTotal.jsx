...
const ShippingTotal = () => {
  ...
  const sub = (id) => {
    let newValu = data.map((el) => {
      return el.id === id ? { ...el, quantity: el.quantity - 1 } : el;
    });
    setData(newValu);
    postUpdate(id, newValu);
  };
  const plus = (id) => {
    let newValu = data.map((el) => {
      return el.id === id ? { ...el, quantity: el.quantity + 1 } : el;
      ...
      {data.map((datas) => (
        <div key={datas.id} style={{ width: "95%", margin: "auto" }}>
          ...
          >
            ...
            <div>
              <button onClick={() => sub(datas.id)}>-</button>
              <button>{datas.quantity}</button>
              <button onClick={() => plus(datas.id)}>+</button>
            </div>
            <div>
...
export default React.memo(ShippingTotal);
